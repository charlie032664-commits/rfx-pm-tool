...
# Rendered right after Step 3 (Analyze RFQ) and before Step 4 (Review Requirements).

if mode == "Select Existing Case":
    with st.expander("Advanced option: Improve requirement wording with AI", expanded=False):
        st.markdown(
...
st.caption("Generate or refresh compliance_matrix.xlsx using the latest reviewed or cleaned "
           "requirements and saved compliance responses.")

# ── v1.4 M5b: explicit Generate Compliance Matrix action. Re-runs the existing
# export_excel.py — no LLM, no re-extraction, no full pipeline. Picks the latest
# available requirement source by priority (reviewed → clean → mock → raw) and
# merges saved Step 5 responses. ──
_gm_dir = RUNS_DIR / selected_case
_gm_source = None
for _gm_fname in ("requirements_reviewed.json", "requirements_clean.json",
                  "requirements_mock.json", "requirements.json"):
    _gm_cand = _gm_dir / _gm_fname
    if _gm_cand.exists():
        _gm_source = _gm_cand
        break
...
if _gm_source is None:
    st.caption("Please analyze the RFQ first before generating the matrix.")
elif _gm_source.name == "requirements_reviewed.json":
    st.info("Using reviewed requirements.")
elif _gm_source.name == "requirements_clean.json":
    st.info("Using cleaned requirements.")
else:
    st.info(f"Using fallback requirement source: {_gm_source.name}.")

if st.button("Generate Compliance Matrix", type="primary",
             key=f"gen_matrix_{selected_case}",
             disabled=(_gm_running or _gm_lock_active or _gm_source is None),
             help="Run export_excel.py to (re)build compliance_matrix.xlsx from "
                  "the latest reviewed/cleaned requirements + saved responses. "
